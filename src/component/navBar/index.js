  ...
  return (
    <nav className="bg-white  text-black px-4 md:px-6 py-1 flex items-center justify-between shadow-lg transition-colors duration-200">
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        <button 
          className="p-1 hover:bg-primary-hover rounded transition-colors duration-200"
      ...
      <div className="hidden lg:flex flex-1 max-w-2xl mx-8">
        <div className="relative w-full">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-full bg-white border-theme text-gray-800 pl-10 pr-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-theme focus:border-theme transition-colors duration-200"
            suppressHydrationWarning
          />
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        ...
        <button className="hidden md:block p-2 hover:bg-theme-hover rounded-full transition-colors duration-200" suppressHydrationWarning>
          <Settings size={20} />
        </button>
        ...
        <div className="relative">
          <button 
            className="p-2 hover:bg-theme-hover rounded-full transition-colors duration-200 relative"
            onMouseEnter={() => setIsNotificationOpen(true)}
            onMouseLeave={() => setIsNotificationOpen(false)}
            suppressHydrationWarning
          >
            <Bell size={18} />
            <span className="absolute -top-1 -right-1 bg-theme text-white text-xs rounded-full w-4 h-4 md:w-5 md:h-5 flex items-center justify-center">
              3
            </span>
          </button>
          ...
          {isNotificationOpen && (
            ...
            >
              ...
              <div className="p-3 md:p-4 bg-gray-50">
                <button className="w-full bg-theme text-white py-2 rounded-lg hover:bg-theme-hover transition-colors duration-200 text-sm md:text-base" suppressHydrationWarning>
        ...
        <div className="relative hidden md:block">
          <button 
            className="p-2 hover:bg-theme-hover rounded-full transition-colors duration-200"
          ...
          {isActivityOpen && (
            ...
            >
              ...
              <div className="p-4 bg-gray-50">
                <button className="w-full bg-theme text-white py-2 rounded-lg hover:bg-theme-hover transition-colors duration-200" suppressHydrationWarning>
                  View all activity
                </button>
              </div>
            </div>
        ...
        <div className="relative" ref={profileRef}>
          <button 
            className="flex items-center space-x-2 hover:bg-theme-hover rounded-lg p-1 md:p-2 transition-colors duration-200"
            onClick={() => setIsProfileOpen(!isProfileOpen)}
            suppressHydrationWarning
          >
            <div className="w-7 h-7 md:w-8 md:h-8 bg-primary rounded-full flex items-center justify-center">
              <span className="text-gray-800 font-bold text-xs md:text-sm">JB</span>
            </div>
          </button>
          ...
          {isProfileOpen && (
            ...
            >
              <div className="p-3 md:p-4 border-b">
                ...
                <div className="mt-3 md:mt-4 flex space-x-1">
                  {timeFilters.map((filter) => (
                    <button
                      key={filter}
                      className={`px-2 md:px-3 py-1 rounded text-xs md:text-sm transition-colors duration-200 ${
                        filter === 'today' 
                          ? 'bg-theme text-white' 
              ...
              <div className="p-2 border-t">
                <button className="w-full bg-danger text-white py-2 rounded-lg hover:bg-danger-hover transition-colors duration-200 flex items-center justify-center space-x-2 text-sm md:text-base" suppressHydrationWarning>
                  <LogOut size={18} />
                  <span>Logout</span>
                </button>
              </div>
